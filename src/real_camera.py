...
json_file = open('../data/results/cnn_model.txt','r')
...
def overlay_memeface(probs):
    if max(probs) > 0.8:
        ...
        return '../data/meme_faces/{}-{}.png'.format(emotion, emotion)
    else:
        index1, index2 = np.argsort(probs)[::-1][:2]
        emotion1 = emotions[index1]
        emotion2 = emotions[index2]
        return '../data/meme_faces/{}-{}.png'.format(emotion1, emotion2)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        print probs
        filename = overlay_memeface(probs)
