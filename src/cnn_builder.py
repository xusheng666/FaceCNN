...
from logModel import save_model, save_config, save_result
import numpy as np
import time
import sys
...
def logging(model, starttime, batch_size, nb_epoch, conv_arch, dense, dropout, X_shape, y_shape, train_acc, val_acc,
            dirpath):
    now = time.ctime()
    model.save_weights('../data/weights/{}'.format(now))
    save_model(model.to_json(), dirpath)
    save_config(model.get_config(), dirpath)
    save_result(starttime, nb_epoch, conv_arch, dense, dirpath)
